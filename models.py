...
class ResponseFormat(BaseModel):
    ...
    format_type: Literal[
        "rating_scale", "multiple_choice", "binary", "open_ended", "other",
...
class Item(BaseModel):
    ...
    item_id: int = Field(description="Unique id, not shared with any AuxiliaryItem.")
    item_text: str = Field(description="Verbatim wording of the question.")
    response_format_id: int = Field(description="format_id of the ResponseFormat this item uses.")
    language: str = Field(description="ISO 639-1 code of the original item text (e.g. 'en', 'de', 'es').")
...
class AuxiliaryItem(BaseModel):
    ...
    item_id: int = Field(description="Unique id, not shared with any Item.")
    item_text: str = Field(description="Verbatim wording of the question.")
    ...
    language: str = Field(description="ISO 639-1 code of the original item text.")
    is_translated: bool = Field(
        default=False,
...
class ScaleItemMembership(BaseModel):
    ...
    item_id: int = Field(description="item_id of the referenced Item.")
...
class Scale(BaseModel):
    ...
    scale_id: int = Field(description="Unique integer id within the survey.")
    scale_name: str = Field(description="Name as printed in the instrument, e.g. 'Extraversion'.")
    construct_name: str = Field(description="Underlying psychological construct the scale measures.")
    ...
    language: str = Field(description="ISO 639-1 code of the original scale name/instructions.")
    is_translated: bool = Field(
        default=False,
...
class Survey(BaseModel):
    ...
    thinking: str = Field(description="LLM scratch field for extraction reasoning; ignored downstream.")
    survey_name: str = Field(description="Full name of the instrument, e.g. 'NEO-PI-R'.")
    instructions: Optional[str] = Field(
        default=None,
    ...
    scales: List[Scale] = Field(description="All scales and subscales, linked to items via ScaleItemMembership.")
    language: str = Field(description="ISO 639-1 code of the original instrument (e.g. 'en', 'de', 'es').")
    is_translated: bool = Field(
        default=False,
